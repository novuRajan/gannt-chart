import { createGridLines, createMonthHeadings, createDateScale, createDivDateScale } from './date-utl.ts';
import { updateTaskStartEndDates } from './updatechart.ts';
import { hideTaskDetails, showTaskDetails, editTask, openAddModal } from './saveEdit.ts';
...
export default class GanttChart {
    ...
    isExceedingDepenentEndDate(sartDate, dependentTask, tasks) {
        const tasksWithDesiredIds = tasks.filter(task => dependentTask.dependencies.includes(task.id));
    ...
    handleMouseUp(taskRect, dependentTask, tasks, dateInfo, allTasks = null) {
        ...
        if (this.isDragging) {
            ...
            const updatedTaskIndex = tasks.findIndex((t) => t.id === dependentTask.id);
            if (updatedTaskIndex !== -1) {
                const newStartDate = new Date(dateInfo.startingDate.getTime() +
